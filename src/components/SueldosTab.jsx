...
export default function SueldosTab({ activeTab, selectedBranchId }) {
  ...
  useEffect(() => {
    try {
      const stored = JSON.parse(localStorage.getItem('blush_sueldos_base_map') || '{}')
      const initialMap = { ...stored }
      personal.forEach(p => {
        if (initialMap[p.id] === undefined) {
          initialMap[p.id] = p.sueldo_base !== undefined && p.sueldo_base !== null ? String(p.sueldo_base) : '0'
        }
      })
      setSueldosBaseMap(initialMap)
    } catch (e) {
      const fallbackMap = {}
      personal.forEach(p => {
        fallbackMap[p.id] = p.sueldo_base !== undefined && p.sueldo_base !== null ? String(p.sueldo_base) : '0'
      })
      setSueldosBaseMap(fallbackMap)
    }
  }, [personal])

  useEffect(() => {
    calculateCommissions()
  }, [selectedMonth, selectedYear, ventas, personal, sueldosBaseMap])

  const handleUpdateSueldoBase = (id, val) => {
    const updated = { ...sueldosBaseMap, [id]: val }
    setSueldosBaseMap(updated)
    try {
      localStorage.setItem('blush_sueldos_base_map', JSON.stringify(updated))
    } catch (e) {}
  ...
  const calculateCommissions = () => {
    ...
    const sorted = comms.sort((a, b) => b.total_pagar - a.total_pagar)
    setComisiones(sorted)

    // Mantener sincronizado el panel de detalle si está abierto
    if (selectedManicuristaDetail) {
      const updatedDetail = sorted.find(c => c.id === selectedManicuristaDetail.id)
      if (updatedDetail) {
        setSelectedManicuristaDetail(updatedDetail)
      }
    }
  }

  // Exportar comisiones y sueldos con formato profesional, colores de marca y bordeado
  const handleExportExcel = async (customColab = null) => {
    try {
      ...
      if (targetColab) {
        ...
        rows.push([makeCell('LIQUIDACIÓN INDIVIDUAL DE SUELDOS Y COMISIONES (40%)', 's', null, {
        ...
        // Fila 4-8: Tarjetas de resumen (KPIs)
        const totalFacturado = detalles.reduce((sum, v) => sum + Number(v.valor_pagado || 0), 0);
        const totalComision = totalFacturado * 0.40;
        const sueldoBase = Number(targetColab.sueldo_base || 0);
        const totalPagar = sueldoBase + totalComision;
        ...
        rows.push([
          makeCell('Sueldo Fijo / Base', 's', null, cardLabelStyle),
          makeCell(sueldoBase, 'n', '"$"#,##0.00', cardValStyle)
        ]);
        rows.push([
          makeCell('Comisión por Servicios (40%)', 's', null, cardLabelStyle),
          makeCell(totalComision, 'n', '"$"#,##0.00', cardValStyle)
        ]);
        rows.push([
          makeCell('TOTAL SUELDO A LIQUIDAR', 's', null, { ...cardLabelStyle, fill: { fgColor: { rgb: 'F0FDF4' } } }),
          makeCell(totalPagar, 'n', '"$"#,##0.00', cardValGreenStyle)
        ]);
        ...
        const footerRow = [makeCell('✦ DOCUMENTO OFICIAL GENERADO POR EL SISTEMA BLUSH BEAUTY STUDIO - CONTROL DE SUELDOS Y COMISIONES ✦', 's', null, {
        ...
        await exportExcelJS(wb, `Liquidacion_${safeName}_${safeMonthName}_${selectedYear}.xlsx`, { sheetName: 'Detalle Comisiones', col: 5, row: 0 });
      } else {
        // ==========================================
        ...
        // ------------------------------------------
        // HOJA 1: RESUMEN GENERAL DE SUELDOS Y COMISIONES
        // ------------------------------------------
        const rowsResumen = [];
        const mergesResumen = [];

        // Fila 0: Logo y Marca BLUSH
        mergesResumen.push({ s: { r: 0, c: 0 }, e: { r: 0, c: 4 } });
        const h0 = [makeCell('BLUSH BEAUTY STUDIO', 's', null, {
          font: { name: 'Segoe UI', size: 17, bold: true, color: { rgb: '748843' } },
          alignment: { horizontal: 'left', vertical: 'middle' }
        })];
        for (let i = 1; i <= 6; i++) h0.push(makeCell('', 's', null, {}));
        rowsResumen.push(h0);

        // Fila 1: Título de reporte
        mergesResumen.push({ s: { r: 1, c: 0 }, e: { r: 1, c: 6 } });
        ...
        // Fila 2: Subtítulo
        mergesResumen.push({ s: { r: 2, c: 0 }, e: { r: 2, c: 6 } });
        ...
        const sumSueldosBase = comisiones.reduce((sum, c) => sum + (c.sueldo_base || 0), 0);
        const sumComision = comisiones.reduce((sum, c) => sum + (c.comision || 0), 0);
        const sumTotalPagar = comisiones.reduce((sum, c) => sum + (c.total_pagar || 0), 0);
        ...
        rowsResumen.push([
          makeCell('Total Sueldos Fijos / Base', 's', null, cardLabelStyle),
          makeCell(sumSueldosBase, 'n', '"$"#,##0.00', cardValStyle)
        ]);
        rowsResumen.push([
          makeCell('Total Comisiones por Servicios (40%)', 's', null, cardLabelStyle),
          makeCell(sumComision, 'n', '"$"#,##0.00', cardValStyle)
        ]);
        rowsResumen.push([
          makeCell('TOTAL GENERAL A LIQUIDAR / PAGAR', 's', null, { ...cardLabelStyle, fill: { fgColor: { rgb: 'F0FDF4' } } }),
          makeCell(sumTotalPagar, 'n', '"$"#,##0.00', cardValGreenStyle)
        ]);
        ...
        rowsResumen.push([
          ...
          makeCell('', 's', null, totalRowStyle),
          makeCell(sumSueldosBase, 'n', '"$"#,##0.00', { ...totalRowStyle, alignment: { horizontal: 'right', vertical: 'middle' } }),
          ...
          makeCell(sumComision, 'n', '"$"#,##0.00', { ...totalRowStyle, alignment: { horizontal: 'right', vertical: 'middle' } }),
          makeCell(sumTotalPagar, 'n', '"$"#,##0.00', { ...totalRowStyle, font: { ...baseFont, bold: true, color: { rgb: '15803D' } }, alignment: { horizontal: 'right', vertical: 'middle' } })
        ]);

        // Pie de página
        const lastRowRes = rowsResumen.length;
        mergesResumen.push({ s: { r: lastRowRes, c: 0 }, e: { r: lastRowRes, c: 6 } });
        const footerRes = [makeCell('✦ DOCUMENTO OFICIAL GENERADO POR EL SISTEMA BLUSH BEAUTY STUDIO - CONTROL DE SUELDOS ✦', 's', null, {
          font: { name: 'Segoe UI', size: 8, italic: true, color: { rgb: '9CA3AF' } },
          alignment: { horizontal: 'center', vertical: 'middle' }
        })];
        for (let i = 1; i <= 6; i++) footerRes.push(makeCell('', 's', null, {}));
        ...
        const safeMonthName = meses[selectedMonth].replace(/\s+/g, '_');
        await exportExcelJS(wb, `Sueldos_Blush_${safeMonthName}_${selectedYear}.xlsx`, { sheetName: 'Resumen General', col: 4, row: 0 });
      }
    } catch (err) {
  ...
  const handleSubmitColaboradora = async (e) => {
    ...
    try {
      ...
      const sueldoNum = Number(formColaboradora.sueldo_base || 0)
      ...
      if (editingPersonalId) {
        ...
        handleUpdateSueldoBase(editingPersonalId, String(sueldoNum))
        setMsgCol({ type: 'success', text: '✓ Colaboradora actualizada con éxito.' })
      } else {
        const nueva = await dataService.registrarPersonal({
          nombre: formColaboradora.nombre.trim(),
          cedula: formColaboradora.cedula.trim() || null,
          cargo: formColaboradora.cargo,
          sueldo_base: sueldoNum,
        ...
        if (nueva?.id) {
          handleUpdateSueldoBase(nueva.id, String(sueldoNum))
        }
        setMsgCol({ type: 'success', text: '✓ Colaboradora agregada con éxito.' })
      }

      setFormColaboradora({ nombre: '', cedula: '', cargo: 'Manicurista', sueldo_base: '0', activo: true })
  ...
  const handleEditColaboradora = (colab) => {
    const rawSueldo = sueldosBaseMap[colab.id] !== undefined ? sueldosBaseMap[colab.id] : (colab.sueldo_base !== undefined ? String(colab.sueldo_base) : '0')
    setEditingPersonalId(colab.id)
    setFormColaboradora({
      nombre: colab.nombre,
      cedula: colab.cedula || '',
      cargo: colab.cargo || 'Manicurista',
      sueldo_base: rawSueldo,
      activo: colab.activo
    })
  }
        ...
        <>
          ...
          <div className="bg-white/85 backdrop-blur-md p-6 rounded-3xl border border-gray-150 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4 animate-fade-in">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-black text-gray-855 uppercase tracking-wide">Calculadora de Sueldos (Fijo + Comisiones 40%)</h2>
                <p className="text-xs text-gray-500 font-medium">Liquida el sueldo base y comisiones de las manicuristas en tiempo real.</p>
              </div>
            </div>
          ...
          {/* DETALLES DE COMISIONES Y SUELDOS */}
          {loading ? (
            <div className="text-center py-16 bg-white rounded-3xl border border-gray-150 shadow-sm animate-pulse">
              ...
              <p className="text-xs text-gray-500 font-bold mt-4">Calculando sueldos y comisiones...</p>
            </div>
          ) : comisiones.length === 0 ? (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-fade-in">
              <div className="lg:col-span-2 bg-white rounded-3xl border border-gray-150 shadow-sm overflow-hidden flex flex-col justify-between">
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse">
                      ...
                      <tbody className="divide-y divide-gray-100 text-xs font-bold text-gray-700">
                        {comisiones.map((c) => {
                          const currentSueldoInput = sueldosBaseMap[c.id] !== undefined ? sueldosBaseMap[c.id] : (c.sueldo_base !== undefined ? String(c.sueldo_base) : '0');
                          return (
                            <tr key={c.id} className="hover:bg-gray-50/40 transition-colors">
                              <td className="py-4 px-5 font-black text-sm text-gray-805">
                                <div>{c.nombre}</div>
                                <div className="text-[10px] text-gray-400 font-semibold">{c.cargo || 'Manicurista'}</div>
                                {!c.activo && (
                                  <span className="inline-block mt-0.5 text-[9px] bg-red-100 text-red-700 px-1.5 py-0.2 rounded font-black uppercase">
                                    Inactiva
                                  </span>
                                )}
                              </td>
                              <td className="py-4 px-3 text-center">
                                <div className="relative inline-flex items-center">
                                  <span className="absolute left-2 text-[11px] text-gray-400 font-bold">$</span>
                                  <input
                                    type="number"
                                    min="0"
                                    step="0.01"
                                    value={currentSueldoInput}
                              ...
                              <td className="py-4 px-5 text-right text-sm font-black">
                                <span className="bg-blush-palmLeaf/10 text-blush-palmLeaf px-2.5 py-1 rounded-lg border border-blush-palmLeaf/20 inline-block">
                                  ${c.total_pagar.toLocaleString('es-EC', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                </span>
                              </td>
                              <td className="py-4 px-3 text-center">
                                <button
                                  ...
                                  title="Ver detalle de servicios y sueldo"
                                >
                ...
                {/* BARRA DE TOTALES (4 MÉTRICAS) */}
                <div className="p-5 border-t border-gray-100 bg-gray-50/70 grid grid-cols-2 md:grid-cols-4 gap-2 text-center text-xs font-black uppercase tracking-wider text-gray-600">
                  <div>
                  ...
                  <div className="border-l border-gray-200 bg-blush-palmLeaf/10 rounded-xl py-1">
                    <span className="block text-[9px] text-blush-palmLeaf font-bold">Total a Pagar (Sueldos)</span>
                    <span className="text-base text-blush-palmLeaf font-black">${comisiones.reduce((sum, c) => sum + c.total_pagar, 0).toLocaleString('es-EC', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-3xl border border-gray-150 shadow-sm p-6 flex flex-col justify-between">
                {selectedManicuristaDetail ? (
                  <div className="space-y-4">
                    <div className="pb-3 border-b border-gray-100 flex justify-between items-start">
                      <div>
                        ...
                        <p className="text-[10px] text-gray-400 font-bold mt-0.5">
                          {selectedManicuristaDetail.cargo || 'Manicurista'} • {meses[selectedMonth]} {selectedYear}
                        </p>
                      </div>
                      <button
                        type="button"
                        ...
                        title="Descargar detalle individual en Excel"
                      >
                    ...
                    {/* Resumen rápido de la colaboradora seleccionada */}
                    <div className="grid grid-cols-3 gap-2 p-3 bg-gray-50 border border-gray-150 rounded-2xl text-center">
                      <div>
                        <span className="block text-[9px] font-black text-gray-400 uppercase">Sueldo Base</span>
                        <span className="text-xs font-black text-gray-700">
                          ${(selectedManicuristaDetail.sueldo_base || 0).toFixed(2)}
                        </span>
                      </div>
                      <div className="border-x border-gray-200">
                        <span className="block text-[9px] font-black text-gray-400 uppercase">Comisión</span>
                        <span className="text-xs font-black text-blush-palmLeaf">
                          ${selectedManicuristaDetail.comision.toFixed(2)}
                        </span>
                      </div>
                      <div>
                        <span className="block text-[9px] font-black text-blush-palmLeaf uppercase">Total Sueldo</span>
                        <span className="text-xs font-black text-emerald-700">
                          ${selectedManicuristaDetail.total_pagar.toFixed(2)}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="pt-3 border-t border-gray-100 flex flex-col gap-1 text-xs font-black text-gray-600">
                      ...
                      <div className="flex justify-between pt-2 border-t border-dashed border-gray-200 text-sm">
                        <span className="text-gray-900 font-black">Total a Liquidar:</span>
                        <span className="text-emerald-700 font-black text-base">
                          ${selectedManicuristaDetail.total_pagar.toLocaleString('es-EC', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex-1 flex flex-col items-center justify-center text-center py-12 text-gray-400 space-y-3">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] font-medium text-gray-455 mt-1 max-w-xs mx-auto text-gray-400">
                        Haz clic en el botón de ojo en la tabla para auditar los servicios y liquidación individual.
                      </p>
                    </div>
                  </div>
                )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-fade-in">
          ...
          <div className="lg:col-span-1 bg-white p-6 rounded-3xl border border-gray-150 h-fit">
            ...
            <form onSubmit={handleSubmitColaboradora} className="space-y-4 text-xs font-bold text-gray-700">
              ...
              <div>
                <label className="block text-[10px] font-black text-gray-500 uppercase tracking-wider mb-1">Sueldo Base Mensual ($)</label>
                <div className="relative">
                  ...
                  <input
                    ...
                    step="0.01"
                    placeholder="0.00"
                    value={editingPersonalId ? '' : formColaboradora.sueldo_base}
                    onChange={(e) => setFormColaboradora({ ...formColaboradora, sueldo_base: e.target.value })}
                    className="w-full pl-7 pr-3 py-2.5 bg-gray-50 border-2 border-gray-250 focus:border-blush-palmLeaf focus:bg-white rounded-xl outline-none transition-all text-xs text-gray-700"
                    disabled={!!editingPersonalId}
                  />
                </div>
                <p className="text-[10px] text-gray-400 mt-1 font-medium">Fijo mensual (adicional a las comisiones del 40%).</p>
              </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        step="0.01"
                        placeholder="0.00"
                        value={formColaboradora.sueldo_base}
                        onChange={(e) => setFormColaboradora({ ...formColaboradora, sueldo_base: e.target.value })}
                        className="w-full pl-7 pr-3 py-2.5 bg-gray-50 border-2 border-gray-250 focus:border-blush-palmLeaf focus:bg-white rounded-xl outline-none transition-all text-xs text-gray-700"
                      />
                    </div>
          ...
          <div className="lg:col-span-2 bg-white p-6 rounded-3xl border border-gray-150 flex flex-col">
            <div className="mb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-3">
              <div>
                ...
                <p className="text-xs text-gray-400 font-medium">Buscador, sueldos base y control de estado de colaboradoras.</p>
              </div>
              ...
              <table className="w-full text-left text-sm border-collapse">
                ...
                <tbody className="divide-y divide-gray-50 text-gray-700 font-bold text-xs">
                  {filteredPersonal.map((colab) => {
                    const currentBase = sueldosBaseMap[colab.id] !== undefined ? Number(sueldosBaseMap[colab.id] || 0) : Number(colab.sueldo_base || 0);
                    return (
                  ...
                  {filteredPersonal.length === 0 && (
                    <tr>
                      <td colSpan={5} className="py-12 text-center text-xs text-gray-400 font-bold bg-white">
